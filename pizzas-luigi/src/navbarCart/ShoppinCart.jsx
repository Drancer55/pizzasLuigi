...
// Contenido del carrito de compras
    ...
    const { shoppinCart, deleteMultiPizza } = useContext(AuthContext);

    useEffect(() => {
        setOrders(
            shoppinCart.reduce((previus, current) => previus + current.quantity, 0)
        )
    }, [shoppinCart]);
    // Se calcula el precio total de los productos
    const Total = shoppinCart.reduce((previus, current) => previus + current.quantity * current.price, 0)
    ...
    return (
        <div className={styles.cartContainer}>
            ...
            >
                ...
                {!cartOpen && (
                    <div className={styles.productsNumber}>{orders}</div>
                )}
            </div>

            {shoppinCart && cartOpen && (
                <div className={styles.cart}>
                    ...
                    {shoppinCart.length === 0 ? (
                        ...
                        <div className={styles.productsContainer}>
                            {shoppinCart.map((food, i) => (
                                <PizzaCart key={i} food={food} />
                            ))}
                        </div>
                    )}
                    <div>
                        ...
                        <div className={styles.count}>
                            <h5 className={styles.Total}> Total: ${Total >= 1 ? Total + ".00" : "00.00"}</h5>
                        </div>
                    </div>
                </div>
            )}
